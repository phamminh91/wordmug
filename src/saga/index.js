...
function* watchLoadDefinition() {
  yield* takeEvery(type.LOAD_DEF_REQ.REQ, loadDefinition);
}
...
function* chooseWord() {
  ...
  console.log('saga:chooseword', idx);
  yield [
...
function* watchChooseWord() {
  yield* takeEvery(type.CHOOSE_WORD_REQ.REQ, chooseWord);
}
...
function* watchObservedWord() {
  yield* takeEvery(type.RECORD_PROGRESS_REQ.REQ, observedWord);
}
...
function* watchUpdateTags() {
  yield* takeEvery(type.UPDATE_TAGS_REQ.REQ, updateTags);
}
